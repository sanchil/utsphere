...
\]
Collecting red and green is not yet yellow, and not yet a poem. Yellow is a later reading of the bowl (paint). A poem is another reading: an ordered path $T$. The same bowl may be read more than one way. Loading is not meaning.
...
Paint is a side reading of the same bowl, not a second name for $T$:
\[
\mathrm{Mix}(\{\mathrm{red},\mathrm{green}\}) = \mathrm{yellow}.
\]
One may collect and read in a single gesture; the algebra does not force the merge. This principle is an axiom of \emph{order of acts}, not a new formula for a 4-tuple.
