...
%
% [U1] Naming collision: Mix is written as "$T_{\mathrm{paint}}$"
%      (Axiom 5 heading and body, and again in Sec. 6/9). This reuses
%      the letter T for something structurally different from the
%      real transformation operator $T_w$ of Axiom 6: $T_w$ is a
%      one-argument rotation+phase-shift parameterized by an actor
%      state w, while Mix is a two-argument phasor combination of two
%      peer states. Since this whole revision arc has been about
%      removing exactly this kind of ambiguity, give Mix its own
%      symbol (e.g. $M$ or $\boxplus$) instead of a T-subscript, so a
%      reader can never wonder whether "$T_{\mathrm{paint}}$" composes
%      with $T_w$ the way two ordinary $T$'s do (it doesn't -- Mix
%      isn't a member of the group $T_w$ lives in).
%
% [U2] Idempotence framing tension: the abstract and Sec. 1.2 lead with
%      "identical states do not double ($A \oplus A = A$)" as a
%      headline property of the algebra. That is true, but only of
%      $\oplus$ (trivial: it's a set union). The operator doing the
%      actually interesting geometric work, Mix, is NOT idempotent in
%      general: Mix(A,A) at the same phase gives z' = 2r e^{i chi}, so
%      r' = min(1, 2r) -- it genuinely doubles r until the cap clips
%      it. That's much closer to ordinary counting-with-a-ceiling than
%      to a phase-cancellation algebra. Worth either (a) adding one
%      sentence at Axiom 5 / the Mix definition noting Mix(A,A) grows
%      until saturation and is therefore distinct from $\oplus$'s
%      idempotence, so a reader doesn't assume the headline "no
%      doubling" claim describes Mix, or (b) rephrasing the abstract's
%      claim to say which operator has which property.
%
% [U3] The general parallel-transport map used in the offset recipe
%      (transport_{m->w}(v), Eq. near \eqref{eq:sphere_exp}) is invoked
%      by name and demonstrated in one special-case worked example
%      (the equatorial check, where m, w, and the pole happen to make
%      the transport a simple rotation about the polar axis) but is
%      never written as its own general equation. Given the rest of
%      this section is already at equation-level rigor (log map, exp
%      map both spelled out), add the general formula: rotate the
%      tangent vector by the geodesic angle theta = arccos(n_m . n_w)
%      about the axis (n_m x n_w)/||n_m x n_w||, and note the axis is
%      undefined when n_m = +/-n_w (antipodal/coincident source and
%      target need a separate case).
%
% [U4] Division remains explicitly undefined ("left open"), which is
%      honest, but it does mean "arithmetic" in the title currently
%      names three operators (oplus/ominus, T, offset) plus a Mix
%      reading -- no multiplication-inverse. Not a defect, but worth a
%      one-line acknowledgment in the abstract or intro that division
%      is open future work, so the title's promise is scoped up front
%      rather than only discovered in Sec. 6.
% =====================================================================
...
and sort $\tau$.
Pattern Arithmetic is the algebra of such units. It proceeds in three acts --- collapse (the bowl, $\oplus$), transform (the path, $T$), result --- so that identical states do not double ($A \oplus A = A$), a gathered unit may be removed ($A \ominus A = \emptyset$, black), and the same ingredients may form different ordered expressions. Anti-phase cancellation to black is a Mix reading, not a gather. Words and numbers occupy the same sphere under different encodings and meet in a document by order and typed action, not by blending. The aim is a single bounded register that carries overlapping identities, structural paths, and mixed prose without a discrete grid.
\end{abstract}
...
\end{itemize}
Pattern Arithmetic is the algebra of these units. Identical states do not double ($A \oplus A = A$). A gathered unit may be removed ($A \ominus A = \emptyset$). Rotations of the sphere preserve the unit. Opposite-phase Mix is a later reading; it is not $\oplus$.
...
1+1=2,
\qquad
\mathrm{Mix}(\mathrm{red},\mathrm{green})=\mathrm{yellow},
\qquad
A\oplus A=A,
\qquad
T_a(T_b(x))\neq T_b(T_a(x)).
...
$\mathrm{Mix}$ & What composite state results? \\
...
\begin{itemize}
    ...
    \item \textbf{Three verbs on that unit:} $\oplus$ collects identities ($A\oplus A=A$); $T$ walks an ordered path; offset copies a relation. Schoolbook $+$ is reserved for $\mathtt{num}$.
\end{itemize}
...
Collecting further distinct shells no longer enlarges a unit past
$r=1$. The stage looks white because the palette is on. The catalogue
...
% TODO [U1] See master punch list. "T_paint" reuses the letter T for
% a two-argument phasor combination, structurally unlike the
% one-argument rotation T_w of Axiom 6. Consider a distinct symbol.
\subsection{Axiom 5: Mix as a reading ($T_{\mathrm{paint}}$)}
\label{subsec:axiom_interference}
A blended 4-tuple appears only when a path \emph{reads} the bowl.
Write $\mathrm{Mix}=T_{\mathrm{paint}}$. Same place first; then a
...
black.
% TODO [U2] Worth a sentence here: Mix(A,A) at equal phase gives
% z' = 2r e^{i chi}, r' = min(1, 2r) -- Mix genuinely grows with
% repeated identical input, up to the r=1 cap, unlike the idempotent
% $\oplus$ that the abstract's headline "no doubling" claim describes.
% A reader who has only read the abstract could otherwise expect Mix
% itself to be idempotent, which it is not except at saturation.
Equal unit paints at $0^\circ$ and $120^\circ$ give
$z'=e^{i\pi/3}$, so
\[
\mathrm{Mix}(1\angle 0^\circ,\,1\angle 120^\circ)
=(1,\,\theta,\,\phi,\,60^\circ)
\quad\text{(yellow)}.
\]
...
$\mathrm{Mix}:\mathcal{S}\times\mathcal{S}\to\mathcal{S}$.

$\oplus$ does not run this rule. Two shells in a bowl have no
resultant angle until $\mathrm{Mix}$ (or another path) is applied.
...
hue. Paint is Mix, a later reading. Analogies such as ruler-man-king are \emph{not} gathers; they are deferred to the offset operator of Section~\ref{sec:analogical_offset}. Gather $\oplus$ does not, and structurally
...
the roles of $\oplus$ and of an explicit cast.
...
Offset is a third verb. It is not $\oplus$ (no blend) and not $T$
...
marked as such. The full recipe carries $v$ along the short geodesic
$m\to w$ so the heading stays a ground heading.
% TODO [U3] "transport_{m->w}(v)" is named and used above, and later
% demonstrated in one worked example (the equatorial check), but is
% never given as its own general equation -- unlike log/exp just
% above, which are both spelled out. Add the general formula: rotate
% v by theta = arccos(n_m . n_w) about axis
% (n_m x n_w)/||n_m x n_w||, and flag the degenerate case
% n_m = +/- n_w (axis undefined; source and target coincide or are
% antipodal) as needing a separate convention.
...
$\log_{\hat n_m}(\hat n_k)=(0,\pi/6,0)$. Carrying that vector a
quarter-turn about $z$ gives $(-\pi/6,0,0)$. The exponential at
$\hat n_w$ lands at
\[
...
place) yields two shells, not an angle. Applying
$\mathrm{Mix}=T_{\mathrm{paint}}$ and
...
is left open.
% TODO [U4] This is the honest place this admission lives, but a
% reader only learns "arithmetic" here means three operators plus a
% Mix reading (no division) by reaching Sec. 6. Consider one line in
% the abstract or Sec. 1.2 scoping this up front, e.g. "division is
% open future work," so the title's promise is calibrated before the
% reader invests in the axioms.
...
no measurement postulate: $\oplus$ and Mix mint a third unit or black;
they do not collapse a superposition onto an eigenstate. Related work
...
commutative mix is reused in spirit. What is added is a bounded unit
...
is a point, $r$ is not a concentration $\kappa$, and $\oplus$, $T$, and
