...
\end{itemize}
A single state at a single direction may carry a color-phase fine enough
to match a 24-bit palette: on the order of $16$ million distinguishable
identities, not $16$ million simultaneous features. Combination still
returns one unit.
...
A discrete matrix grid is the wrong box for such a unit. Traditional matrix representations of continuous state spaces suffer from three structural bottlenecks:
\begin{itemize}
    ...
    \item \textbf{Sparse occupancy of a large address book:} A high-dimensional matrix that holds only a few nonzero numbers still reserves a huge grid of empty cells. When the natural object is a handful of unit states, one may store those states on $\mathcal{S}$ directly instead of allocating the empty book. This does not abolish sparse matrices for graphs or tables; it avoids using one as a warehouse for a few arrows.
\end{itemize}
...
\begin{itemize}
    \item \textbf{High-Density Superposition:} Storing overlapping state vectors concurrently within a single bounded spatial manifold. Phase-color supplies a large menu of identities at one position; it does not place millions of independent features there at once.
