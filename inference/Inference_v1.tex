...
\]
This decoder is that diagram's missing branch, made explicit:
$\oplus+(\text{Markov or }T)$. Two distinct claims follow, and they
should not be run together. \emph{Drop-in address}: wherever a
downstream pipeline needs one vector per token, $E_L$ can occupy that
slot; an atom's $(r,\hat n)$, or a listed inner bundle, reads where a
learned high-dimensional embedding otherwise would. \emph{Same verbs,
foreign net}: a transformer may still be what fills $T$, or what
trains the transition table above --- unreplaced. The claim in
either case is a shared address and a shared verb set, never that
