...
Built this way, $\mathrm{ref}_c$ is a Mixed state like any other, and
inherits Mix's own information loss along with it: it reports what
class $c$'s witnesses resemble in aggregate, never what any one
witnessed episode's own bowl looked like beneath it. Comparing a new
bowl's verdict against $\mathrm{ref}_c$ therefore compares one
collapsed state to another; it does not, and by construction cannot,
recover the composition of either (companion paper, \S7.2, where
this is shown to be ill-posed for the algebra itself, not merely
unbuilt). What comparison buys instead is a stated confidence rather
than a forced exact answer: $\mathrm{ref}_c$ is built from witnessed
cases, so a poor match against it is a reason to doubt, not a
wrong-looking right answer with no internal sign of being wrong.
