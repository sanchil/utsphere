...
and sort $\tau$.
Pattern Arithmetic is the algebra of such units. It proceeds in three acts --- collapse (the bowl, $\oplus$), transform (the path, $T$), result --- so that identical states do not double ($A \oplus A = A$), opposite phases cancel ($A - A = 0$), and the same ingredients may form different ordered expressions. Words and numbers occupy the same sphere under different encodings and meet in a document by order and typed action, not by blending. The aim is a single bounded register that carries overlapping identities, structural paths, and mixed prose without a discrete grid.
\end{abstract}
