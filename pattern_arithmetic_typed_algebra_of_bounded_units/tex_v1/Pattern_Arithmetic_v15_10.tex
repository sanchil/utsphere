...
named exception, not a change to the default.}:
strength $r \in [0, 1]$,
...
seat. Units left alone descend an energy until those seats stop,
resting in one camp or several. Collapse~$\to$~$T$~$\to$~result is
...
The teaching atom is this 4-tuple: one arrow, $N=1$. Axiom~\ref{subsec:axiom_v1_metrics} allows an atom to hold $N$ such arrows under one closure, each still capped at $r_i\le 1$, plus one uncapped derived sum that is not itself a unit. That bundle does not replace Equation~\eqref{eq:unit_truth}. Every informational arrow in the bundle is an instance of this axiom. Operators that need a single pair $(r,\hat n)$ read the derived sum as a weight or a direction; as the actor of $T$ in three dimensions, only after the cast of Axiom~\ref{subsec:axiom_v1_metrics}, and beyond three dimensions not from the derived sum at all, but from $N_{\mathrm{dim}}-2$ of the bundle's own orthonormal arrows directly (\S\ref{subsec:transform_higher_dim}). They do not mint a second kind of unit.
...
second is to raise the ambient dimension, $S^2\to S^{N-1}$
...
reaches outside the bounded stage of Axiom~\ref{subsec:axiom1}. The
...
absence of a favoured direction that Axiom~\ref{subsec:axiom_null}
...
(Axiom~\ref{subsec:axiom_null}): at $r_A=0$ the energy is exactly
zero, no relation being formed with something absent, which is what
black already means elsewhere. Whether a structure of three or more
...
The teaching atom of Axiom~\ref{subsec:axiom1} is one 4-tuple
($N=1$). An atom may also be a \emph{bundle}: $N$ informational
vectors $v_i=r_i\hat n_i$, each independently an instance of that
same 4-tuple and each still capped at $r_i\le 1$. $N$ is the atom's
info number. This is not a second species of unit and not a field
on the globe. It is several teaching atoms held under one closure. At most one
further, \emph{derived} vector may sit alongside them in the same
closure unit:
\[
...
admissibility of its own (Axiom~\ref{subsec:axiom0_admissibility}),
the same status the companion paper\footnote{\emph{Pattern
Arithmetic: Verdicts from Bowls and Paths}, which builds inference
...
(Axiom~\ref{subsec:axiom_assembly}), a camp body's alignment ---
...
Mix's own uncapped sum (Axiom~\ref{subsec:axiom_interference}), or,
...
complement. At $N_{\mathrm{dim}}=3$ that fixed subspace is one
vector; a circle's fixed point stacks into a sphere's
$1$-dimensional axis, and a sphere's axis stacks into a $4$-sphere's
$2$-dimensional axis plane, the pattern continuing by one dimension
at each step. Three dimensions is not where axes stop needing more
than a point --- it is where they still fit in one vector.
...
(Axiom~\ref{subsec:axiom_v1_metrics}) rather than as one $\hat n_w$:
\begin{equation}
    T_w(x)
    \;=\;
    \Bigl(
...
as before. This requires $N\ge N_{\mathrm{dim}}-2$: an atom acting as
...
Axiom~\ref{subsec:axiom0_admissibility}. A sort inadmissible for
...
Every operator before Axiom~\ref{subsec:axiom_assembly} works
...
Axiom~\ref{subsec:axiom1}, in the same way step metadata sits on the
...
the clean decline of Equation~\eqref{eq:enclosure_motion}. Rest is
the state in which seats no longer move.
...
finely the descent is sampled, not where the descent ends: the same
starting seats integrated at $\delta = 0.02$, $0.01$, $0.005$, and
$0.002$ reach resting configurations whose bodies agree to within
$5\times 10^{-12}$. Rest is a property of
Equation~\eqref{eq:assembly_energy} and the starting seats, not of
how the path to it was computed. A step chosen too coarsely relative
to $\varepsilon_{AB}$ will misbehave near close approach, in the
ordinary way of any fixed-step scheme; that is a limit on the
...
therefore reach rest simultaneously, each one a \emph{camp}. The
number of camps is not chosen in advance; it is whatever the law and
the starting seats produce.

For each camp $G_k$ that has stopped, its \emph{body} is
\begin{equation}
    \vec P_{c,k}
...
because anything resists. There is correspondingly no core, no halo,
...
and let external seats rest; the resting camps have bodies, not
names. Naming a camp is a later comparison, stated in the companion
paper on verdicts.
...
\paragraph{Open: dimensionality, $S^2 \to S^{N-1}$.}
...
$\mathrm{SO}(N)$ as an abstract group --- associativity and
invertibility are group-theoretic facts about $\mathrm{SO}(N)$ for
any $N$, not special to $N=3$ --- but that is a statement about the
...
crowd; raising $N$ for a bowl of two or three atoms relieves nothing,
...
$T$ is no longer part of this TODO:
\S\ref{subsec:transform_higher_dim} restates it for general
$N_{\mathrm{dim}}$, checked directly rather than assumed, invertible
and non-commutative up to $N_{\mathrm{dim}}=12$ and exact at
$N_{\mathrm{dim}}=3$. What remains a TODO is everything else: Mix,
Assembly, and the offset transport formula's own proof steps, each
still only checked at $N_{\mathrm{dim}}=3$ despite the Riemannian
statements above being expected to hold more generally.
...
Axiom~\ref{subsec:axiom1}, and $\hat n$ remains a pattern's semantic
