...
physics, biology, or computation.
Storms, forests, and a cancellation cartoon later in the paper
...
map to $(\oplus,\boxplus,T,\text{offset})$ exists at a stated
fidelity, and most physical domains do not offer one.
...
approximation named). If no such $\varphi_\tau$ exists, $\tau$ is
\emph{geometrically inadmissible}: it may still occupy a point on
...
\begin{itemize}
    ...
    has an exact, invertible chart (Section~\ref{subsec:clock_encoding}):
    $\varphi_{\mathrm{num}}$ is a bijection, not an approximation.
    $\mathtt{word}$ makes no claim at all --- $T$ on a word is
    bookkeeping (order, presence), never a statement about
    linguistic physics. Both are silently admitted at this inert
    level; nothing further needs to be said to use them.
...
\begin{tabular}{lccc}
\hline
Sort & Native law $\circ_\tau$ & $\varphi_\tau$ exists? & Tier \\
\hline
$\mathtt{num}$   & schoolbook $+$          & yes, exact bijection & 0 (default) \\
$\mathtt{word}$  & none claimed             & n/a, no claim made   & 0 (default) \\
$\mathtt{note\text{-}pitch}$ & pitch-class sum & yes, near-exact  & 1 (explicit) \\
$\mathtt{color}$ & additive light mixing   & yes, model-dependent & 1 (explicit, state which) \\
$\mathtt{snowflake}$ & $D_{6h}$ diffusion-limited growth & no & inadmissible for Tier 1; loadable at Tier 0 \\
\hline
\end{tabular}
\caption{Admissibility is a property of the sort, decided before
Table~\ref{tab:type_table}'s operator legality is even asked. A sort
...
two bags, the arithmetic everyone already has); two objects placed
