...
\begin{itemize}
    ...
    \item \textbf{Magnitude ($0 \leq r \leq 1$):} Strength of that one unit, capped so the arrow remains a single unit of truth. Not a density for the whole sphere.
...
\label{subsec:axiom1}
A pattern is one state, not a field painted over the globe. The atom is a
4-tuple
\begin{equation}
    x = (r,\,\theta,\,\phi,\,\chi),
    \qquad
    ...
    \label{eq:unit_truth}
\end{equation}
The cap $r \le 1$ applies to \emph{that} arrow. It is not a demand that
all arrows on $\mathcal{S}$ integrate to one. Two states may occupy
...
\subsection{Atmospheric storm (costume)}
Four seats --- wind, heat, moisture, eye --- collapse to one bowl
$R$. Two paths $T_{\mathrm{deepen}}$ and $T_{\mathrm{clear}}$ are
two weathers from the same parts. The storm is the walk, not a
fifth pin and not a claim about vorticity.
