...
A discrete matrix grid is the wrong box for such a unit.
A voxel cube of resolution $N$ stores $O(N^3)$ cells, which
grows steeply with resolution, and two signals sent to the
same cell overwrite. The sphere is offered instead as one
bounded object that can carry overlapping identities by
direction and phase, without enlarging a lattice.

The next section turns these names into axioms.
