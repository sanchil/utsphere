...
% section) are RESOLVED as of v14_19/v14_24. The remaining naming
% and scoping items U1--U4 are RESOLVED in this file.
%
% [U1] RESOLVED: Mix is now $\boxplus$, not $T_{\mathrm{paint}}$.
%      Mix is a two-argument reading of peer states; $T_w$ remains
%      the one-argument rotation+phase-shift. They do not compose
%      as two ordinary $T$'s.
%
% [U2] RESOLVED: the no-doubling headline is now scoped to gather
%      ($A\oplus A=A$). Axiom 5 notes that Mix is not idempotent:
%      $A\boxplus A$ gives $z'=2r e^{i\chi}$, $r'=\min(1,2r)$.
%
% [U3] RESOLVED: transport_{m\to w}(v) is now \eqref{eq:transport}:
%      rotate v by \theta_{mw}=arccos(n_m·n_w) about
%      (n_m\times n_w)/||n_m\times n_w||. Coincident source and
%      target: identity. Antipodal: axis undefined, left as a
%      separate convention. The equatorial check cites the general
%      formula.
%
% [U4] RESOLVED: abstract and the intro operator table now say
%      division as unmixing is left open, with a pointer to
%      Section~\ref{subsec:division_open}. Inverse rotation of $T$
%      remains defined; a Mix inverse is not claimed.
% =====================================================================
...
idempotent gather, $T$ is typed action, and $x-a+b$ is the analogical
