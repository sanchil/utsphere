...
\begin{itemize}
\item \textbf{Dynamics, force verified, mass and mechanism still
      open.} $F=-\nabla U$ was checked directly and correctly
      recovers the true position-space separation axis. Turning
      force into motion needed two further pieces, and testing both
      changed the shape of this gap considerably:
      \begin{itemize}
      \item \emph{Mass.} Three candidates, none chosen: $m=r$
            (invokes the equivalence principle, since $r$ already
            plays gravitational coupling in $U_{AB}$); $m=r^2$
            (amplitude-to-energy, following directly from the
            Chladni mapping, but in tension with $U_{AB}$'s linear
            coupling); $m=r^3$ (literal spherical volume, the
            mildest assumption, requires reading $r$ as a physical
            radius). For a multi-vector atom, mass is proposed as
            $\sum r_i$ over informational vectors only, reusing the
            already-established energy quantity rather than
            introducing a new one.
      \item \emph{Softening.} The energy's $1/d^3$ term diverges at
            $d=0$; a fixed-step integrator was shown to become
            numerically unstable near close approach. Resolved:
            $\varepsilon_{AB}=r_A+r_B$, derived rather than chosen,
            following the same $r$-as-radius reading used for mass.
            Verified as coupled to timestep size --- too small an
            $\varepsilon$ relative to $dt$ reproduces the same
            instability.
      \item \emph{Damping, removed rather than derived.} An external
            $-bv$ term was tested and found unnecessary once genuine
            multi-body dynamics is used: a five-atom, undamped,
            energy-conserving simulation showed real contraction
            followed by real re-expansion (spread $2.76\to1.62\to
            8.04$ over 2000 steps), matching how real self-gravitating
            systems evolve --- core tightening alongside gradual
            member ejection, not permanent settling. No external
            energy loss belongs in a vacuum, gravity-only system; the
            settling-and-scattering behavior emerges from the bodies
            themselves.
      \end{itemize}
      Net honest state: the equation is right, softening is now
      derived, damping was shown not to belong at all --- but what
      the system was originally pictured as producing (one dominant,
      stable shape) is not what verified multi-body gravity actually
      gives. It gives ongoing, realistic evolution instead.
\item \textbf{Mass segregation, partially verified.} In any group of
      mutually attracting bodies, stronger members are expected to
      sink toward the group's own center of mass while weaker
      members are pushed toward the periphery --- a real, observed
      astrophysical phenomenon (checked against current literature
      on globular cluster dynamics). The five-atom test above is
      consistent with this picture (temporary contraction, then
      ejection-driven expansion) but did not isolate or confirm the
      strength-sorted radial ordering itself; still a named
      prediction, now with suggestive rather than confirming
      evidence.
\item \textbf{Multi-vector generalization, largely closed.} Every
      ...
      not in general.
\item \textbf{Identical-atom multiset.} $A$ assembling with an exact
      copy of itself needs a count, not a set. Unaddressed.
\item \textbf{Symmetric or actor/patient.} Polarity
      (\S\ref{subsec:assembly_features}) supplies the
      \emph{criterion} --- equal strength suggests a mutual bond,
      unequal strength suggests an actor and a patient, mirroring
      $T$'s own asymmetry --- but the energy formula itself is
      perfectly symmetric under exchange of $A$ and $B$ regardless
      of polarity. The behavioral \emph{consequence} of that
      asymmetry (which atom moves more, which adapts to the other)
      can only be realized once dynamics exists.
\end{itemize}
...
\subsection{Axiom V4: Motion without mass}
\label{axiom:v4}
Position changes by moving directly along the energy gradient, with
no mass term and no momentum carried between steps:
\[
