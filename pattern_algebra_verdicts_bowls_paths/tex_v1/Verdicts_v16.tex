...
toward $0$ as they scatter. $\mathcal{V}_{\text{atom}}$, atom
volume, is strictly smaller than $E_{\text{atom}}$ whenever any
$r_i<1$, a forced consequence of the $r\le 1$ cap rather than an
assumption: cubing a number below $1$ always shrinks it, exactly as
treating $r$ as a literal radius and $r^3$ as the volume it encloses
would predict. $\gamma_{\text{atom}}$, mean vector volume, is the
average per-vector share of that total. Neither is used in
Axiom~V4; atom volume, in particular, is deliberately excluded from
motion (Axiom~V4) and exists here purely as a descriptive metric of
the atom. Earlier drafts of this axiom called $\mathcal{V}_{\text
{atom}}$ ``mass'' and $\gamma_{\text{atom}}$ ``density'' --- both
retired as misleading, since neither participates in momentum,
kinetic energy, or any calculation the word ``mass'' would
ordinarily license.

A fourth metric answers a question the other three cannot. Atom
...
minimum of $-\tfrac{1}{2}\sum_i r_i^2$, growing only as $N$, which
follows directly from $\lvert V_{\text{derived}}\rvert^2 =
\sum_i r_i^2 + 2\sum_{i<j} r_ir_j(\hat n_i\cdot\hat n_j) \ge 0$.
...
A fifth metric does for magnitude what $\alpha_{\text{atom}}$ does
for direction. $E_{\text{atom}}$ and $\mathcal{V}_{\text{atom}}$
both report a total and are silent on how that total is spread: two
atoms can carry near-identical energy ($2.200$ against $2.150$ in a
...
size. $\mathcal{V}_{\text{atom}}$ rather than $E_{\text{atom}}$
