...
claims. They were written Verdicts-native --- numbered V1--V4 rather
than continuing the companion paper's own Axiom 0--5 --- but all
four have since crossed over: V1's informational vectors and seven
metrics as the companion paper's own Axiom~7; V2's seats as its
Enclosure \S~Seats; V3's assembly energy as its Axiom~6; V4's motion
as its Enclosure \S~Rest. The V-numbering here is kept for this
paper's own internal cross-references and status tracking, not
because the material remains Verdicts-only --- what follows is
restated at the level this paper's own exposition needs, with full
derivations and worked examples left to the companion paper's own
versions. Two items named in \S\ref{subsec:assembly_open} --- the
identical-atom multiset, and the full behavioral realization of the
actor/patient question --- remain genuinely open and are not
resolved by anything below.

\subsection{Axiom V1: Informational and derived vectors}
\label{axiom:v1}
An atom's informational vectors are $N$ ordinary units of Axiom~1
(companion paper), each independently capped at $r\le 1$; $N$ is the
atom's info number. Alongside them, at most one \emph{derived}
vector may be present, $V_{\text{derived}}=\sum_{i=1}^N v_i$, the
raw, unnormalized sum, computed after and never including itself.
$V_{\text{derived}}$ is uncapped and excluded from $N$, needing no
$\tau$-justified admissibility of its own, the same status already
held by $\bar R_c$ and $z_{G_c}$ (\S\ref{par:reference_atom}). Its
magnitude and direction read as an effective $(r,\hat n)$ pair, $r =
\lvert V_{\text{derived}}\rvert$, $\hat n=V_{\text{derived}}/r$,
standing in for the atom wherever a single vector is required; for
$N=1$ it coincides with the atom's own vector.

Seven scalar metrics follow from the same $N$ informational vectors,
each answering a different question, none used elsewhere below
unless named:
\[
...
\]
atom strength, volume, and mean vector volume: $S_{\text{atom}}$ is
the correct denominator for atom-level coherence, $\lvert
V_{\text{derived}}\rvert/S_{\text{atom}}$; $\mathcal{V}_{\text{atom}}$
is deliberately excluded from Motion (Axiom~V4) and is a purely
descriptive metric here. (Earlier drafts called these ``mass,''
``density,'' and ``atom energy'' --- retired; none behaves as those
words would license, and a genuine energy notion exists below and
is not $S_{\text{atom}}$.)
\begin{equation}
    \mathcal{E}_{\text{atom}} \;=\; \sum_{i=1}^N r_i^2,
    \label{eq:information_energy}
\end{equation}
information energy, standing on its own rather than derived from
strength by squaring it, the way variance stands on its own rather
than as squared standard deviation: the atom's intrinsic energy
prior to any question of how its own vectors align, distinct from
bowl energy (a sum across separate witnesses, \S\ref
{par:reference_atom}).
\begin{equation}
    \alpha_{\text{atom}} \;=\;
...
\end{equation}
internal alignment, the strength-weighted mean cosine similarity
across all internal pairs, separating active cancellation
($\alpha_{\text{atom}}<0$) from mere non-reinforcement, which atom
coherence's ratio-of-magnitudes cannot: it reads $0$ for both.
\begin{equation}
    \sigma_{\text{atom}} \;=\;
    \frac{N^2}{N-1}\cdot\operatorname{Var}(s_1,\ldots,s_N),
    \qquad s_i = \frac{r_i^3}{\mathcal{V}_{\text{atom}}},
    \qquad N\ge 2,
    \label{eq:volume_concentration}
\end{equation}
($\sigma_{\text{atom}}=0$ by convention at $N=1$) volume
concentration, $0$ when volume is spread evenly and $1$ when a
single vector holds all of it, at every $N$ --- normalized this way
specifically so that a perfectly even atom always reads $0$
regardless of $N$, unlike the largest-share or Herfindahl
alternatives.
\begin{equation}
    \sigma_{\text{shell}}(w) \;=\;
    \begin{cases}
...
\end{equation}
shell concentration at width $w$ (default $0.1$, binning by
$\lfloor r_i/w\rfloor$ into $K$ occupied shells with populations
$p_k$), closing a blind spot $\sigma_{\text{atom}}$ has by
construction: many vectors independently arriving near the same
magnitude are invisible to a per-vector share, which reads a
genuinely clumped population as more even than a scattered one until
this is added.

Full derivations, the variance-identity proof behind
$\mathcal{E}_{\text{atom}}$, worked numerical examples, and the
verification behind each formula --- the antipodal-pairs case for
$\alpha_{\text{atom}}$, the Herfindahl rejection for
$\sigma_{\text{atom}}$, the $K=1$ removable singularity for
$\sigma_{\text{shell}}$ --- now live in the companion paper's own
Axiom~7, into which this axiom's content was ported; restated here
only at the depth this paper's own exposition needs.
