...
\item \textbf{Dynamics, closed.} Motion follows Axiom~V4: gradient
      descent, no mass, no momentum. Verified stable at $N=2$, $3$,
      and $8$, and confirmed as the deliberate, final rule rather
      than one candidate among several --- mass-weighted motion was
      tested as the alternative and shown to destabilize precisely
      where mass ratios grow extreme, which settled the choice on
      stability grounds rather than convenience.
\item \textbf{Symmetric or actor/patient, closed at criterion,
      ...
      exactly the mass-weighted motion Axiom~V4 excludes on
...
\begin{itemize}
\item \textbf{Mass segregation, superseded.} Named as a prediction
      while dynamics was still mass-based: stronger members sinking
      toward a group's center, weaker members toward the periphery,
      as in real globular-cluster relaxation. Once Axiom~V4 settled
      motion as mass-free, this prediction's own mechanism no longer
      applies --- confirmed directly, equal force with no mass
      dividing it gives identical movement regardless of strength.
      Recorded here as a real idea that depended on a choice later
      reversed, not as something still to be verified.
...
Two scalar metrics follow directly from the same $N$ informational
vectors, each answering a different question and neither used by
any other axiom below unless explicitly named:
\[
E_{\text{atom}} = \sum_{i=1}^N r_i, \qquad
M_{\text{atom}} = \sum_{i=1}^N r_i^3, \qquad
\rho_{\text{atom}} = \frac{M_{\text{atom}}}{N}.
\]
$E_{\text{atom}}$ is the correct denominator for atom-level
coherence, $\lvert V_{\text{derived}}\rvert / E_{\text{atom}}$, the
structural twin of $\bar R_c$ one level down --- verified to read
$1.0$ for perfectly aligned informational vectors and fall smoothly
toward $0$ as they scatter. $M_{\text{atom}}$ is strictly smaller
than $E_{\text{atom}}$ whenever any $r_i<1$, a forced consequence of
the $r\le 1$ cap rather than an assumption: cubing a number below
$1$ always shrinks it. $\rho_{\text{atom}}$ is the average
per-vector mass. None of the three is used in Axiom~V4; mass, in
particular, is deliberately excluded from motion (Axiom~V4) and
exists here purely as a descriptive metric of the atom.
...
mass term appears, by deliberate design rather than by an
unresolved choice: mass (Axiom~V1) is a real, useful metric of an
atom's own structure, but makes no sense as an input to motion, and
is excluded from it.
\[
...
pair, and no mass divides the resulting step, two atoms of any
polarity move by identical magnitude toward each other, regardless
of strength. Weighting each atom's step by its own mass was tested
directly as an alternative and produces the intended differential
movement, but destabilizes into oscillation precisely where $M_{
\text{atom}}$'s ratio is most extreme --- confirmed at a $9\times$
strength difference, giving a $729\times$ mass ratio and a
demonstrably non-monotonic trajectory, against the clean monotonic
decline of the massless rule under the same pair. Axiom~V4 is
therefore stated without mass on stability grounds, not merely
convention; a differential-motion mechanism is not a matter this
axiom leaves open, since any such mechanism reintroduces the
