...
\item \textbf{Group energy.} $\sum_{\text{pairs}} U_{ij}$, the
      ...
      energy --- computable today for any bowl, though see
      \S\ref{subsec:assembly_open} on what it does not yet account
      for.
\end{itemize}
...
\begin{itemize}
\item \textbf{Dynamics.} $F=-\nabla U$, then $F=ma$: the generic
      engine that turns an energy function into actual motion over
      time. Never built. A single hand-rolled numerical gradient
      step was used to test predictions (below), but no iterated,
      general capability exists.
\item \textbf{Group reconciliation.} How three or more atoms settle
      into one physically consistent joint configuration when they
      cannot each occupy their own private optimal axis
      simultaneously. Named, argued for, not built. Real physics'
      answer --- iterated energy minimization, not a closed formula
      --- is known and cited, but not implemented here.
\item \textbf{Mass segregation, a named but unrealized consequence.}
      In any group of mutually attracting bodies, stronger members
      are expected to sink toward the group's own center of mass
      while weaker members are pushed toward the periphery --- a
      real, observed astrophysical phenomenon (verified against
      current literature on globular cluster dynamics), mapped here
      by treating $r$ as the coupling strength gravity assigns to
      mass. This is a long-timescale, statistical consequence of
      repeated dynamical encounters, not a static result; it
      requires \emph{dynamics}, run over many iterations, to
      actually occur --- named here as a prediction the framework
      makes, not as something demonstrated.
\item \textbf{Multi-vector generalization, conditional.} If atomic
      number (\S\ref{subsec:assembly_features}) is adopted as real
      --- multiple vectors per atom, multiple simultaneous bonds ---
      every formula above needs to become vector-to-vector rather
      than atom-to-atom. Not attempted, and not a gap at all unless
      atomic number itself is adopted.
