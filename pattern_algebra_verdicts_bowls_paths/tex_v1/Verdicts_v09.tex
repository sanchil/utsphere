...
\subsection{Four cells, one operation}
\label{subsec:four_cells}

Comparison (\S\ref{subsec:comparison_rule}) is the one operation
behind everything in this section and the last. What varies is only
what a produced state is checked against, and when --- two axes,
crossing to give four cases rather than one or three:

\begin{center}
\begin{tabular}{lll}
\hline
 & now (classification) & later (forecast) \\
\hline
prototype (compress via $\boxplus$) &
...
fault line as $\boxplus$ and $T$. Prototype compression is available
wherever $\boxplus$'s composites are guaranteed to share a direction;
exemplar-keeping is what remains wherever that guarantee fails --- a
$T$-landing (\S\ref{subsec:walk_classifier}), or, in the one open
cell, a whole sequence with no defined distance yet.

Three of the four cells are specified and checked on worked numbers;
none should be read as more finished than that. Prototype
...
is warranted. Exemplar classification
(\S\ref{subsec:walk_classifier}) is complete for what it does ---
walking new evidence and comparing the landing against kept
witnesses --- with compression into a single exemplar reference
blocked separately, not left unfinished. The immediate forecast
